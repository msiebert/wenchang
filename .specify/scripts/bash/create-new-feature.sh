...
LINEAR_ID=$(printf '%s' "$LINEAR_SOURCE" | grep -Eio 'AIE-[0-9]+' | head -1 | tr '[:lower:]' '[:upper:]')

# Generate branch name. When a Linear ID is present, strip it from the source
# text first so it doesn't consume one of the few words kept in the slug
# (generate_branch_name only keeps the first 3-4 meaningful words).
SLUG_DESCRIPTION="$FEATURE_DESCRIPTION"
if [ -n "$LINEAR_ID" ]; then
    SLUG_DESCRIPTION=$(printf '%s' "$FEATURE_DESCRIPTION" | sed -E "s/${LINEAR_ID}//gi" | sed -E 's/^[[:space:]]+|[[:space:]]+$//g')
fi
...
if [ -n "$SHORT_NAME" ]; then
    ...
    BRANCH_SUFFIX=$(generate_branch_name "$SLUG_DESCRIPTION")
fi
...
if [ -n "$LINEAR_ID" ]; then
    # Linear-ID naming already computed BRANCH_NAME and SPEC_DIR_NAME above;
    # still enforce GitHub's 244-byte branch-name limit.
    ORIGINAL_BRANCH_NAME="$BRANCH_NAME"
    BRANCH_NAME=$(printf '%s' "$BRANCH_NAME" | cut -c "1-$MAX_BRANCH_LENGTH" | sed 's/-$//')
